...
async def _other_search(self, interaction):
    if yt_dlp is None:
        await interaction.response.send_message(
            "Other Search is unavailable because **yt-dlp** is not installed.",
            ephemeral=True,
        )
        return
    ...
    async def on_submit(self, interaction):
        ...
        try:
            ...
            await interaction.followup.send(
                embed=discord.Embed(
                    title="🌐 Other Search • Queued Next",
                    description=(
                        f"**{title}**\n*{artist}*\n\n"
                        f"**Source:** `{source}`\n"
                        "**Position:** `Next`\n\n"
...
def patch_external_search():
    JuiceVaultPanelView._other_search = _other_search
