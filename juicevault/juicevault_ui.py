...
class JuiceVaultPanelView(discord.ui.View):
    ...
    def __init__(self, panel, guild_id):
        ...
        next_button = discord.ui.Button(label="Next", emoji="⏭️", style=discord.ButtonStyle.primary, custom_id=f"juicevault:next:{guild_id}")
        next_button.callback = self._next
        self.add_item(next_button)

        refresh_button = discord.ui.Button(label="Refresh", emoji="🔄", style=discord.ButtonStyle.secondary, custom_id=f"juicevault:refresh:{guild_id}")
        refresh_button.callback = self._refresh
        self.add_item(refresh_button)

        stop_button = discord.ui.Button(label="Stop", emoji="⏹️", style=discord.ButtonStyle.danger, custom_id=f"juicevault:stop:{guild_id}")
    ...
    async def _next(self, interaction: discord.Interaction):
        ...
        if self.guild_id not in cog.tasks or not event or not voice or not voice.is_playing():
            ...
            return
        if event.is_set():
            await interaction.followup.send("Un skip este deja în curs.", ephemeral=True)
            return
        event.set()
        voice.stop()
...
class JuiceVaultUI(commands.Cog):
    ...
    def _make_embed(self, guild_id):
        ...
        if track:
            ...
            embed.add_field(name="Requested", value=f"`{requested_size}` piese", inline=True)
            embed.add_field(name="Queue", value=f"`{queue_size}` piese în așteptare", inline=True)
    ...
    async def _get_panel_message(self, guild_id, create=False, channel=None):
        ...
        if not create:
            return None
        message = await target.send(embed=self._make_embed(guild_id), view=JuiceVaultPanelView(self, guild_id))
    ...
    async def update_panel(self, guild_id):
        ...
        if message is None:
            return
        try:
            await message.edit(embed=self._make_embed(guild_id), view=JuiceVaultPanelView(self, guild_id))
        except (discord.NotFound, discord.Forbidden, discord.HTTPException):
            pass
    ...
    async def _maybe_create_after_start(self, ctx):
        ...
        try:
            ...
            if message:
                await message.edit(embed=self._make_embed(ctx.guild.id), view=JuiceVaultPanelView(self, ctx.guild.id))
